...
func (s *serviceImpl) predicateBasedDryRunPolicy(ctx context.Context, cancelCtx concurrency.ErrorWaitable, request *storage.Policy) (*v1.DryRunResponse, error) {
	...
	clusterProvider := detection.NewClusterLabelProvider(s.clusters)
	namespaceProvider := detection.NewNamespaceLabelProvider(s.namespaces)

	compiledPolicy, err := pkgDetection.CompilePolicyWithProviders(request, clusterProvider, namespaceProvider)
	if err != nil {
